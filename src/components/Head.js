...
import { autoSuggestion } from '../utils/content.js'
import {cacheResult} from '../utils/CacheSlice.js'
...
const searchQuery=useSelector(store=>store.search)
    ...
    useEffect(() => {
        const Timer = setTimeout(() => {
            if (searchQuery[value]) {
                setSuggestions(searchQuery[value]);
            } else {
    ...
    const getSuggestion = async () => {
        ...
        const data = await fetch(autoSuggestion + value)
    ...
    return (
        <div className='h-16 w-full top-0 fixed z-10 bg-white shadow-lg flex items-center p-2 justify-between'>
            <div className="left-side-header flex">
                <button onClick={() => toggelMenuBar()}>  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 5.25h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5" />
                </svg></button>
             
                 <a href="/">
                     <div className='h-2 w-[8vw] ml-2'><svg viewBox="0 0 97 20" focusable="false">
            ...
            <div className="middle-side-header flex items-center">

                <input className="border-2 rounded-lg border-r-0 w-[30vw] h-8  " type="text" value={value} onChange={(e) => setvalue(e.target.value)} />
                <button><svg className="w-10 h-8 border-2 rounded-r-full " xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                </svg>
                </button>
                {suggestions.length !== 0 && <div className='bg-white w-[30vw] border-2 absolute top-12 h-32 overflow-y-scroll'>
                    <ul>
                        {suggestions.map((item, i) => {
                            return <li key={i} className='border-b-2'>🔍{item}</li>
                        })}
                    </ul>
